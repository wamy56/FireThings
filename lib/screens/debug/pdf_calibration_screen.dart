...
/// Screen for calibrating PDF field positions.
///
/// Generates a PDF with field boundaries drawn on the actual template
/// so you can see exactly where fields are positioned.
class PdfCalibrationScreen extends StatefulWidget {
  const PdfCalibrationScreen({super.key});

  @override
  State<PdfCalibrationScreen> createState() => _PdfCalibrationScreenState();
}

class _PdfCalibrationScreenState extends State<PdfCalibrationScreen> {
  ...
  void initState() {
    super.initState();
    _template = PdfFormTemplates.iqModificationCertificate;
    _generateDebugPdf();
  }
  ...
  void _exportCoordinates() {
    final coords = _template.fields.map((f) => {
      'id': f.id,
      'label': f.label,
      'x': f.x,
      'y': f.y,
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: _pdfBytes != null
                ? () => TemplatePdfService.sharePdf(_pdfBytes!, 'debug_field_positions.pdf')
                : null,
            tooltip: 'Share PDF',
          ),
        ],
          ...
          : _pdfBytes == null
              ? Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(AppIcons.danger, size: 48, color: Colors.red),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: PdfPreview(
                        ...
                        allowPrinting: true,
                        allowSharing: true,
                        pdfFileName: 'debug_field_positions.pdf',
                      ),
                    ),
                  ],
                ),
